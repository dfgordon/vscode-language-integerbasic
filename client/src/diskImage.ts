...
export class A2KitTool extends lxbase.LangExtBase {
	...
	async select(rows: Array<string> | string): Promise<boolean> {
		...
		if (fname && fname == dotDotRow) {
			...
			return true;
		} else if (fname && fname.startsWith("INT")) {
			this.imgPath.push(this.currPath() + fname.substring(FNAME_START));
			return false;
		} else if (!fname) {
	...
	async getIntegerFile() {
		...
		try {
			await lxbase.request<null>("integerbasic.disk.mount", [uri[0].fsPath]);
	
			this.operation = "get";
			this.imgPath = [""];
			
			let response: string[] | string = await lxbase.request<Array<string>>("integerbasic.disk.pick", [this.currPath(),["int"]]);
			if (response.length==0) {
				vscode.window.showErrorMessage('no integer program files or directories were found');
				return;
			}
			...
			do {
				...
				if (selecting) {
					response = await lxbase.request<Array<string>>("integerbasic.disk.pick", [this.currPath(), ["int"]]);
				}
			} while (selecting);
			response = await lxbase.request<string>("integerbasic.disk.pick", [this.currPath(), ["int"]]);
	...
	async putIntegerFile() {
		...
		try {
			...
			let response = await lxbase.request<string[]>("integerbasic.disk.pick", [this.currPath(),[]]);
			let selecting;
			do {
				selecting = await this.select(response);
				response = await lxbase.request<string[]>("integerbasic.disk.pick", [this.currPath(),[]]);
			} while (selecting);
			response = await lxbase.request<string[]>("integerbasic.disk.pick", [this.currPath(), null]);
			...
			if (existingFileList.includes(fname)) {
				...
				await lxbase.request<null>("integerbasic.disk.delete", [this.currPath()]);
			}
			const verified = lxbase.verify_document();
			if (!verified) {
			...
			await lxbase.request<null>("integerbasic.disk.put", [this.currPath(), verified.doc.getText()]);
