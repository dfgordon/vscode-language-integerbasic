...
export class A2KitTool extends lxbase.LangExtBase
{
	...
	/**
	 * Parse a disk catalog to extract directories and Integer files
	 * @param catalog the disk catalog with the file system's formatting
	 * @returns two arrays, first with directories, second with Integer files
	 */
	parse_catalog(catalog: string): [string[], string[]] | undefined {
		...
		if (lines.length>3 && lines[1].length>11 && lines[1].substring(0, 11) == "DISK VOLUME") {
			for (const line of lines.slice(3)) {
				if (line.length>7 && line[1] == "I") {
					files.push("(INT)  "+line.slice(7));
				}
			}
			return [dirs, files];
		}
		if (lines.length>5 && lines[3].length>21 && lines[3].substring(0, 21) == " NAME            TYPE") {
			for (const line of lines.slice(5)) {
				...
				if (line.length>20 && line.substring(17,20) == "INT") {
					files.push("(INT)  "+line.slice(1,15).trim());
				}
			}
			return [dirs, files];
		}
	...
	async save_code(buf: Buffer) {
		this.a2kit_bin2txt(["put", "-d", this.fsPath, "-f", this.imgPath[this.imgPath.length - 1], "-t", "itok"], this.finish_put.bind(this), buf);
	}
	...
	async detokenize(tokens: Buffer) {
		...
		// program can go anywhere after ZP for this purpose
		const startAddr = 256;
		const himem = startAddr + prog_bytes.length;
		img_messg[202] = startAddr % 256;
		img_messg[203] = Math.floor(startAddr / 256);
		img_messg[76] = himem % 256;
		img_messg[77] = Math.floor(himem / 256);
		for (let i = 0; i < prog_bytes.length; i++) {
			img_messg[startAddr + i] = prog_bytes[i];
		}
		const code = await client.sendRequest(vsclnt.ExecuteCommandRequest.type,
			{
	...
	async runOperation() {
		if (this.operation == "get") {
			this.a2kit_txt2bin(["get", "-d", this.fsPath, "-f", this.imgPath[this.imgPath.length - 1], "-t", "itok"], this.detokenize.bind(this), undefined);
	...
	async select(raw_catalog: string) {
		...
		if (this.operation=="get" && dirs.length == 0 && files.length == 0 && this.imgPath.length == 1) {
			vscode.window.showErrorMessage('no integer program files or directories were found, raw data: '+raw_catalog);
			return;
		}
		...
		if (fname && fname == "(DIR)  ..") {
			...
			this.recursiveSelection();
		} else if (fname && fname.substring(0, 7) == "(INT)  ") {
			this.imgPath.push(this.imgPath[this.imgPath.length - 1] + fname.substring(7));
			this.runOperation();
		} else if (!fname) {
			abortMessage();
